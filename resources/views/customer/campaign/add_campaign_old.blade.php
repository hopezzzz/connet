                     ...
                     <div class="form-group md-form clearfix">
                       <h6 class="text-left" style="font-size: 14px; color: red;">INSTRUCTIONS / TIPS: <br> Please add the Contacts to receive calls for this campaign. </h6>
                        ...
                        <div class="add-input">
                           ...
                           <button class="btn btn-mdb-color btn-rounded add_Contact add float-right openModal"  data-target="#add-new-tag" type="button">Add new contact </button>
                           @if(isset($campData['contactCount']) && $campData['contactCount'] > 0)
                           <button class="btn btn-mdb-color btn-rounded getContacts add" data-url="{{URL('/customer/get-contacts')}}" type="button">Add an existing contact  </button>
                           @endif
                        </div>
                     </div>
                     <div class="col-md-12">
...
function hightLight (element, start, end,tagName , id) {
    ...
    str = str.substr(0, start) +
        "<mark data-name='"+tagName.replace("\"", "")+"'  class='campaignTag' data-id='"+id+"'>" +
         str.substr(start, end - start) + "</mark>" + str.substr(end);
