$(document).ready(function() {
	...
	$(document).on('click', '.deleteRecordBtn', function(event)
	{
		...
		else
		{
            ...
            var itBevar = 0;
            if (tableTrCount == 0) {
              itBevar = $('.recordsTable').find('table thead tr th').length
              $('.recordsTable').find('tbody').html('<tr><td colspan="'+itBevar+'">No Contact Found</td></tr>')
            }
					...
					if(responstruee.ajaxPageCallBack)
					{
...
// $(document).ready(function() {
    ...
    jQuery('.nav-link').each(function() {
			...
			if (hrefurl)
			{
				for (var i = 0; i < ur.length; i++)
				{
					...
					if (toActive > -1) {
							document.title = jQuery(this).text();jQuery(this).parent().addClass('active');break;
					};
				}
			}
     })
    ...
    // $(document).on('mouseout', '.campaignTag', function(event) {
    //     event.preventDefault();
		//
		// 		console.log(event);
...
function closePOP() {
    ...
    jQuery(document).on('click','.fa-clipboard',function(eve) {
        var copyText = document.getElementById("campTextId").innerHTML;
